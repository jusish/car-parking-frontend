...
import { AlertDialog, AlertDialogAction, AlertDialogFooter, AlertDialogCancel, AlertDialogDescription, AlertDialogHeader, AlertDialogTitle, AlertDialogContent, AlertDialogTrigger } from '@/components/ui/alert-dialog';
...
export function OrdersList() {
  ...
  const columns: Column<SlotOrder>[] = [
    ...
    },
    {
      header: 'Status',
      accessorKey: 'status',
      cell: (order) => <StatusBadge status={order.slotOrderStatus} />,
      sortable: true,
    },
    {
      header: 'Created At',
      accessorKey: 'createdAt',
      cell: (order) => format(new Date(order.createdAt || ''), 'MMM dd, yyyy HH:mm'),
      sortable: true,
    },
    {
      header: 'Actions',
  ...
  return (
    <>
      ...
      <Dialog open={createModelOpen} onOpenChange={setCreateModelOpen}>
       ...
       <Dialog open={orderDEtailsModelOpen} onOpenChange={setOrderDEtailsModelOpen}>
        <DialogContent>
          <OrderDetails selectedId={selectedId} open={orderDEtailsModelOpen} openChange={setOrderDEtailsModelOpen}/>
        </DialogContent>
      </Dialog>
