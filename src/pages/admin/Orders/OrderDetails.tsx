...
export function OrderDetails({open,openChange,selectedId}:{open:boolean,openChange:(open:boolean)=>void,selectedId:string}) {
  const id=selectedId
        ...
        <Card className="col-span-2 border-none w-full">
          <CardHeader>
            <div className="flex justify-between items-center">
              ...
              <StatusBadge status={order.slotOrderStatus} />
            </div>
            <CardDescription>Created on {format(new Date(order?.createdAt || ''), 'MMMM dd, yyyy')}</CardDescription>
          </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="bg-muted/50 p-4 rounded-md space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm flex items-center gap-1">
                      <Calendar className="h-3 w-3" />
                      {format(new Date(order.updatedAt || order.createdAt || ''), 'PPpp')}
                    </p>
                  </div>
                </div>
              </div>
                ...
                {order.slot ? (
                  <div className="bg-muted/50 p-4 rounded-md space-y-3">
                    <div>
                      ...
                      <p className="text-sm">{order.slot.slotNumber}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">Size</p>
                      <SlotSizeBadge size={order.slot.slotSize} />
                    </div>
                    <div>
                      <p className="text-sm font-medium">Status</p>
                      <StatusBadge status={order.slot.slotStatus} />
                    </div>
                  </div>
                ) : (
                  <div className="bg-muted/50 p-4 rounded-md text-center">
                    <p className="text-sm text-muted-foreground">Slot information not available</p>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm flex items-center gap-1">
                      ...
                      {order.slotVehicle.vehiclePlateNumber}
                    </p>
                  </div>
                  {order.slotVehicle && (
                    <>
                      <div>
                        ...
                        <p className="text-sm">
                          {order.slotVehicle.vehicleBrand} {order.slotVehicle.vehicleModel} ({order.slotVehicle.vehicleYear})
                        </p>
                      </div>
                      <div>
                        <p className="text-sm font-medium">Type</p>
                        <p className="text-sm">{order.slotVehicle.vehicleType}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium">Color</p>
                        <p className="text-sm">{order.slotVehicle.vehicleColor}</p>
                      </div>
                    </>
                  )}
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm flex items-center gap-1">
                        ...
                        {order.slotCustomer.firstName} {order.slotCustomer.lastName}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">Email</p>
                      <p className="text-sm">{order.slotCustomer.email}</p>
                    </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Select
                value={status}
                onValueChange={(value) => setStatus(value as SlotOrderStatus)}
              >
                <SelectTrigger>
